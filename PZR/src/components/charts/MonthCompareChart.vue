    ...
    <div class="panel-header">
      ...
      <div class="legend">
        <span class="legend-item current">
          <span class="legend-dot"></span> This month
        </span>
        <span class="legend-item prev">
          <span class="legend-dot"></span> Last month
        </span>
      </div>
    </div>
...
const props = defineProps({
  currentData: { type: Array, default: () => [] }, // [{ label, revenue }]
  prevData:    { type: Array, default: () => [] }, // [{ label, revenue }]
})
...
const chartData = computed(() => ({
  ...
  datasets: [
    {
      label:           'This month',
      data:            props.currentData.map(d => d.revenue),
      borderColor:     '#4ADE80',
      backgroundColor: ctx => makeGradient(ctx, '#4ADE80'),
      fill:            true,
      tension:         0.35,
      pointRadius:     0,
      pointHoverRadius: 4,
    ...
    {
      label:           'Last month',
      data:            props.prevData.map(d => d.revenue),
      borderColor:     '#555',
      backgroundColor: ctx => makeGradient(ctx, '#555'),
      fill:            true,
      tension:         0.35,
      pointRadius:     0,
      pointHoverRadius: 4,
...
.legend-item.current .legend-dot { background: #4ADE80; }
.legend-item.prev    .legend-dot { background: #555; }
