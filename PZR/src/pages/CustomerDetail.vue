<template>
  <div v-if="customer">
    ...
    <div class="cust-bar">
      ...
      <RouterLink to="/dashboard/customers" class="back-btn">← Back</RouterLink>
    </div>

    <!-- metrics -->
    <div class="metrics">
      <MetricCard label="Purchases"    :value="customer.purchases" />
      <MetricCard label="Total Spent"  :value="fmt(customer.spent)" />
      <MetricCard label="Avg Purchase" :value="fmt(avgPurchase)" />
      <MetricCard label="Discount"     :value="customer.discount + '%'" />
    </div>

    <!-- spending chart -->
    <RevenueChart
      :week-data="spendWeek"
      :month-data="spendMonth"
      :year-data="spendYear"
      color="#A78BFA"
    />
...
const avgPurchase = computed(() =>
  customer.value && customer.value.purchases > 0
    ? Math.round(customer.value.spent / customer.value.purchases)
    : 0
)

// Deterministic first visit date based on customer ID
...
// Simulated spend chart data scaled from global monthly revenue
...
// Fabricated purchase history based on customer data
const PRODUCTS = [
...
const purchases = computed(() => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    result.push({
      id:      i + 1,
      date:    dateStr,
      product: PRODUCTS[(Number(c.id) + i) % PRODUCTS.length],
      qty,
      amount:  base * qty,
    })
  }
...
function tierLabel(t) {
  return { gold: 'Gold', silver: 'Silver', bronze: 'Bronze', none: 'New' }[t] ?? t
}
</script>
...
.cust-meta { font-size: 11px; color: var(--text-3); margin-top: 3px; }
...
.tag-gold   { background: rgba(251,191,36,0.12);  color: var(--amber); }
.tag-silver { background: rgba(148,163,184,0.12); color: #94A3B8; }
.tag-bronze { background: rgba(180,130,90,0.12);  color: #C4956A; }
.tag-none   { background: rgba(85,85,85,0.15);    color: var(--text-3); }
